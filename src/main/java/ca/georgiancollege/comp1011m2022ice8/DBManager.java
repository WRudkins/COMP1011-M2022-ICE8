...
public class DBManager
{
    ...
    public int insertVector2D(Vector2D vector2D) throws SQLException {
        int ID = -1; // if this method returns -1 - it means that something went wrong
    ...
    public ArrayList<Vector2D> readVectorTable()
    {
        ...
        String sql = "SELECT vectors.ID, X, Y FROM vectors GROUP BY vectors.ID";
        ...
        )
        {
            ...
            while(resultSet.next())
            {
                ...
                vectors.add( new Vector2D(ID, X, Y));
            }
        }
